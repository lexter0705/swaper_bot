...
class DeDustWorker(TonDexWorker):
    ...
    async def ton_to_token(self, token_ticket_to: str, amount: int):
        ...
        await self.wallet.dedust_swap_ton_to_jetton(token.address, amount, jetton_decimals=token.decimals)

    async def token_to_ton(self, token_ticket_from: str, amount: int):
        token = self.tokens.get_with_truth_ticket(token_ticket_from)
        await self.wallet.dedust_swap_jetton_to_ton(token.address, amount, jetton_decimals=token.decimals)
    ...
    async def token_to_token(self, token_ticket_from: str, token_ticket_to: str, amount: int):
        ...
        await self.wallet.dedust_swap_jetton_to_jetton(token_from.address, token_to.address, amount,
